			...
			array(
				...
				'header_options'        => array(
					...
					'image_shortcode_options'       => array(
						...
						'alt' => esc_html__('Alt Text', 'arprice-responsive-pricing-table'),
...
global $arp_api_url, $arpricelite_class;

$arp_api_url = $arpricelite_class->arpgetapiurl();
$arp_plugin_slug = basename(dirname(__FILE__));

add_filter('pre_set_site_transient_update_plugins', 'arpricelite_check_for_plugin_update');

function arpricelite_check_for_plugin_update( $checked_data ){
	$arp_lite_force_update_check = get_transient('arp_lite_force_update_check');

	if( !empty( $arp_lite_force_update_check ) ){
		global $arp_plugin_slug, $arpricelite_class, $arp_api_url, $wp_version, $arpricelite_version;
		...
		$args = array(
            ...
            'version' => $arpricelite_version,
            'other_variables' => $arpricelite_class->arprice_get_remote_post_params()
        );
		...
		$request_string = array(
            'body' => array(
                ...
                'api-key' => md5( ARPLITE_HOME_URL )
            ),
            'sslverify' => false,
            'user-agent' => 'ARPLITE-WordPress/'.$wp_version.';'.ARPLITE_HOME_URL
        );
		...
		if( isset( $response['access_request'] ) && !empty( $response['access_request'] ) && 'success' == $response['access_request'] ){
            if( isset( $response['access_package'] ) && !empty( $response['access_package'] ) ){
                ...
                if( isset( $update_package ) && is_object( $update_package ) && is_object( $checked_data ) && !empty( $update_package ) ){
                    $checked_data->response[$arp_plugin_slug .'/' . $arp_plugin_slug .'.php'] = $update_package;
                }
            }
        }
	}
...
add_filter( 'plugins_api', 'arprice_lite_plugin_api_call', 11, 3 );

function arprice_lite_plugin_api_call( $res, $action, $args ){
	$arp_lite_force_update_check = get_transient( 'arp_lite_force_update_check' );

	if( !empty( $arp_lite_force_update_check ) ){
		global $arp_plugin_slug, $arp_api_url, $wp_version, $arpricelite_version;

		if ( !isset( $args->slug ) || ( $args->slug != $arp_plugin_slug ) ){
            return $res;
        }

		$args->version = $arpricelite_version;
		...
		if (is_wp_error($request)) {
			$res = new WP_Error('plugins_api_failed', '<p>'.esc_html__('An Unexpected HTTP Error occurred during the API request.', 'arprice-responsive-pricing-table') . "</p> <p><a href='?' onclick='document.location.reload(); return false;>".esc_html__('Try again','arprice-responsive-pricing-table')."</a></p>", $request->get_error_message());
		} else {
			$res = unserialize($request['body']);

			if ($res === false)
				$res = new WP_Error('plugins_api_failed', esc_html__('An unknown error occurred', 'arprice-responsive-pricing-table'), $request['body']);
		}
	}
...
add_action( 'init', 'arprice_lite_validate_plugin_update' );

function arprice_lite_validate_plugin_update(){
	$arp_lite_force_update_check = get_transient( 'arp_lite_force_update_check' );
	...
	if( empty( $arp_lite_force_update_check ) ){
        ...
        if( is_wp_error( $validate_active_plugin ) ){
            ...
            set_transient( 'arp_lite_force_update_check', 1, $expiration_sec );
        }
	}
}