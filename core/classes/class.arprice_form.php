...
class arpricelite_form {
	...
	function arplite_header_image_shortcode( $atts ) {
		...
		$alt_text = isset( $atts['alt'] ) ? sanitize_text_field( $atts['alt'] ) : '';
		$https            = is_ssl() ? 's' : '';
		...
		$alt_attr = $alt_text !== '' ? ' alt="' . esc_attr( $alt_text ) . '"' : '';
		$style = '';
		if ( $open_in_lightbox == 1 ) {
			...
			return "<div class='arp_header_image arp_header_image_lightbox' data-bpopup=\"<iframe class='arp_video_ifr' src='" . esc_url( $image_url ) . "' style='border:0px;margin:0px;" . esc_html( $arpifr_width ) . esc_html( $arpifr_height ) . "'></iframe>\"> 
                    <img " . esc_attr( $width ) . esc_attr( $height ) . $alt_attr .   " src='" . esc_url( $image_url ) . "' class='alignnone arp_video_current_img' style='" . esc_attr( $style_width ) . esc_html( $style_height ) . "'/> 
                    </div>";
		} else {
			return '<div class="arp_header_image"' . ( $style != '' ? ' style="' . esc_attr( $style ) . '"' : '' ) . '><img ' . esc_attr( $width ) . esc_attr( $height ) . $alt_attr .  ' src="' . esc_url( $image_url ) . '" class="alignnone" style="' . esc_attr( $style_width ) . esc_attr( $style_height ) . '" /></div>';
		}
	}
